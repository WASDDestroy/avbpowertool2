...
3. **选择创建模式** — 手动或自动
4. **准备密钥**（手动/自动共用，在收集镜像**之前**）— 向导创建密钥库目录
   `profiles/<id>/keys/`，提示你把 `.pem` 私钥放入该目录，然后自动发现
   （文件名去掉 `.pem` 即 key_id）。手动模式下 key_id 可直接从发现的密钥中
   选择；自动模式下链式分区的公钥解析有已填充的 manifest 可匹配。
5. **分区** — 添加一个或多个分区及其签名设置
